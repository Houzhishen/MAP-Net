...
class FewShotDataset_train(Dataset):
    ...
    def _sample_episode(self):
        ...
        Returns:
            ...
            nKnovel: N way
            nExemplars: K shot in support set
            nEvalExamplesPerClass: K shot in query set
        """
        ...
        Tnovel = []      # query set
        Exemplars = []   # support set
        for Knovel_idx in range(len(Knovel)):
            ids = (nEvalExamplesPerClass + self.nExemplars)
            img_ids = random.sample(self.labels2inds[Knovel[Knovel_idx]], ids) 

            imgs_tnovel = img_ids[:nEvalExamplesPerClass]    # query set
            imgs_emeplars = img_ids[nEvalExamplesPerClass:]  # support set
    ...
    def _creatExamplesTensorData(self, examples):
        ...
        Returns:
            ...
            wordembs: a tensor  [nExemplars, d]
        ...
        for (idx, label) in examples:
            ...
            word = torch.from_numpy(word).float()
            images.append(img)
            wordembs.append(word)
            labels.append(label)
            cls.append(ids)
        images = torch.stack(images, dim=0)
        wordembs = torch.stack(wordembs, dim=0)
        labels = torch.LongTensor(labels)
        cls = torch.LongTensor(cls)
        return images, wordembs, labels, cls


    def __getitem__(self, index):
        Tnovel, Exemplars = self._sample_episode()
        S_imgs, S_words, S_labels, S_cls = self._creatExamplesTensorData(Exemplars)
        Q_imgs, Q_words, Q_labels, Q_cls = self._creatExamplesTensorData(Tnovel)
        return S_imgs, S_words, S_labels, Q_imgs, Q_words, Q_labels, Q_cls
